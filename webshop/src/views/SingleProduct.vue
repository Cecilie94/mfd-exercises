<template>
  <div v-if="product">
    ...
    <h2>hello</h2>
    <button @click="addToCart" :disabled="isProductInCart(product.id)">{{ isProductInCart(product.id) ? 'Already in Cart' : 'Add to Cart' }}</button>
    <button @click="removeFromCart" v-if="isProductInCart(product.id)">Remove from Cart</button>
...
function addToCart() {
  ...
  if (product.value) { // Kontroller om produktet er defineret
